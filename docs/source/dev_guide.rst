...
First activate the ``parrot`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n parrot --explicit > spec-file.txt
...
Make a new version of Parrot in the following steps:
